...
describe('BalancerBoostedStrategy - USDC - Join', function () {
  ...
  const POOL_DAI_USDC_USDT_ADDRESS = '0x7b50775383d3d6f0215a8f290f2c9e2eebbeceb2'

  const LINEAR_POOL_USDC_ID = '0x9210f1204b5a24742eba12f710636d76240df3d00000000000000000000000fc'
  const LINEAR_POOL_USDC_ADDRESS = '0x9210F1204b5a24742Eba12f710636D76240dF3d0'
  ...
  const batchSwap = async (
    ...
    await assets[assetInIndex].connect(from).approve(bVault.address, amount)
    ...
    await bVault.connect(from).batchSwap(
      0, //GIVEN_IN
      swaps,
  ...
  before('load tokens', async () => {
    ...
    linearPoolUSDC = await instanceAt('IBalancerPool', LINEAR_POOL_USDC_ADDRESS)
  ...
  before('deploy strategy', async () => {
    ...
    strategy = await deploy('BalancerBoostedStrategy', [
      vault.address,
      usdc.address,
      bVault.address,
  ...
  it('more gains to recover lost in single token join slipage', async () => {
    ...
    const assets = [pool, linearPoolUSDC, usdc]
  ...
  it('exit 50% strategy', async () => {
    ...
    //rounding issue
    expectWithError(accountValue, strategyShares.mul(strategyShareValue).div(bn(1e18)).add(20))
